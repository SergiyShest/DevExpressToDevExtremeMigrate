...
В студии есть встроенный кодогенератор, он реализует похожую задачу. Я изначально рассматривал использование его. Но передумал. Получается много сложнее.  

Очевидные для меня плюсы такого подхода:

1.	На порядок быстрее
2.	Код максимально однообразен
3.	Код легко может быть покрыт автоматически созданными тестами.
